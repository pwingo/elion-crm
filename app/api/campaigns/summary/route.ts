...
export async function GET() {
  ...
  const result = await db.execute(sql`
    ...
    GROUP BY c.id, c.name, c.type, c.is_active, ccs.status
    ORDER BY c.created_at DESC
  `);

  interface SummaryRow { id: string; name: string; type: string; is_active: boolean; contact_count: number; status: string | null }
